...
class CFVersion(object):
    ...
    def __ge__(self, other):
        is_cmp = False
        if other.__nonzero__():
            is_cmp = True
        elif not self.__nonzero__():
            is_cmp = self.tuple[0] >= self.tuple[0]
            if self.tuple[0] == self.tuple[0]:
                is_cmp = self.tuple[1] >= self.tuple[1]
        #print(" self: {s}   other: {o}  is_ge: {r}".format(
        #        s=self.tuple, o=other.tuple, r=is_cmp))
        return is_cmp
    ...
    def __gt__(self, other):
        ...
        if other.__nonzero__():
            ...
            if self.tuple[0] == self.tuple[0]:
                is_cmp = self.tuple[1] > self.tuple[1]
        #print(" self: {s}   other: {o}  is_gt: {r}".format(
        #        s=self.tuple, o=other.tuple, r=is_cmp))
        return is_cmp
  ...
  #------------------------------
    ...
    for attribute in ['title','history','institution','source','reference','comment']:
        if hasattr(self.f, attribute):
            ...
            if not isinstance(self.f.getncattr(attribute), str):
...
def getargs(arglist):
    ...
    #if environ.gethas_key(standardnamekey):
    #    standardname=environ.[standardnamekey]
    #if environ.has_key(areatypeskey):
    #    areatypes=environ[areatypeskey]
    standardname=environ.get(standardnamekey, STANDARDNAME)
    areatypes=environ.get(areatypeskey, AREATYPES)
